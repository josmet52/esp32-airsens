...
v0.2.2 : 03.04.2024 --> axe des abcisses en jours écoulés et pas en dates
"""
import sys
import socket
...
class AirSensBatGraph:
    ...
    def convert_sec_to_h(self, seconds):
        min, sec = divmod(seconds, 60)
        hour, min = divmod(min, 60)
        return str(int(hour)) + 'h'
    ...
    def get_elapsed_time(self, local):
        ...
        elaps_hm = self.convert_sec_to_h(elapsed_s)
        ...
        for i, local_d in enumerate (locaux.items()):
            ...
            if len(ubat) != 0:
                ...
                legend2 = ax1[plot_place[i]].legend(['Vie batterie: ' + elapsed +
                                                     ' (' + elaps_hm + ") (" + str(n_mes) + " mes)"], loc='upper left')
